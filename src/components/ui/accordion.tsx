...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-3 text-h-4 tb:text-sm-cap mb:text-sm-cap font-normal transition-all text-left [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  ...
  >
    <div
      className={cn(
        "pb-4 pt-0 text-p tracking-wide tb:text-sm-cap mb:text-sm-cap",
        className
      )}
    >
      {children}
